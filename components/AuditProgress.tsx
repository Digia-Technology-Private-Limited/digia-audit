  ...
  return (
    <main className="audit-shell">
      <section className="audit-panel" aria-labelledby="audit-title">
        ...
        {topOpportunity ? <div className="top-outcome"><p className="audit-kicker">#1 problem to act on next</p><h2>{topOpportunity.problemStatement}</h2><p>{topOpportunity.issueType} · {topOpportunity.digiaAddressable ? "Digia addressable" : `Owner: ${topOpportunity.recommendedOwner}`} · {topOpportunity.evidenceCount} supporting reviews</p><Link className="outcome-link" href={`/audits/${auditId}/opportunities/${topOpportunity._id}`}>Inspect evidence and recommendation →</Link></div> : null}
