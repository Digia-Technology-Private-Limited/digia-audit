    ...
    try {
      ...
      const response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          model,
          store: false,
          instructions: "You are the Product Analyst agent. Convert each grounded problem candidate into one diagnosed opportunity. Use the supplied evidence review text to validate the diagnosis and only cite review IDs present in that candidate's evidence. Keep nearly duplicate clusters separate. Diagnose whether the underlying issue is Engineering, UX, Education, Feature Gap, Performance, Data, Support, or Other. Be conservative about Digia addressability: crashes, backend failures, data corruption, and performance failures are not Digia-addressable. Digia can address discoverability, education, and in-product guidance. Propose ranking inputs from 1 to 10; these are inputs only and the application will calculate the score.",
